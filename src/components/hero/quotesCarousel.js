import React from "react"

import "./quotesCarousel.scss"

const QuotesCarousel = () => {

    /* NOTE: put this not an external file/folder */
    const quotes = [
        {
            quote: "It is up to us to allow the obstacles in our live to be the excuse for our failure... or the reason behind our success.",
            author: 'Unknown'
        }, {
            quote: "Easy choices, hard life. Hard choices, easy life.",
            author: "Jerzy Gregorek"
        }, {
            quote: "Passion is the path to peak performance.",
            author: "Anonymous"
        }, {
            quote: "When you decide to be better than you were yesterday and maintain this attitude, you are going to the peak.",
            author: "Israelmore Ayivor"
        }, {
            quote: "The best view comes after the hardest climb.",
            author: "Anonymous"
        }, {
            quote: "Every mountain top is within reach if you just keep climbing.",
            author: "Barry Finlay"
        }, {
            quote: "There are far better things ahead than the ones we leave behind.",
            author: "C.S. Lewis"
        }, {
            quote: "It’s not the mountain we conquer, but ourselves.",
            author: "Sir Edmund Hillary"
        }, {
            quote: "If you aren’t going all the way, why go at all?",
            author: "Joe Namath"
        }, {
            quote: "Each fresh peak ascended teaches something.",
            author: "Sir Martin Convay"
        }, {
            quote: "Between peaks there are always valleys. How you manage your valleys determines how soon you reach your next peak.",
            author: "Spencer Johnson"
        }, {
            quote: "I took a walk in the woods and came out taller than trees.",
            author: "Henry David Thoreau"
        }, {
            quote: "The experienced mountain climber is not intimidated by the mountain, he is inspired by it.",
            author: "William Arthur Ward"
        }

    ]

    return (
        <div id="quoteCarousel" className="footer-item">
            <div className="inner-container">
                {quotes.map( (qts, i) => (
                    <div key={i} className="quote-container">
                        <p className="quote">{qts.quote}</p>
                        <p className="author">{qts.author}</p>
                    </div>
                ))}
            </div>
        </div>
    )
}

export default QuotesCarousel